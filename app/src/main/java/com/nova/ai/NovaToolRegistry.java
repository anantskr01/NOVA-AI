...
public final class NovaToolRegistry {
    ...
    public synchronized boolean register(NovaTool tool) {
        if (tool == null || tool.id() == null || tool.id().trim().isEmpty()) return false;
        if (tools.containsKey(tool.id())) return false;
    ...
    public synchronized JSONArray describe() {
        JSONArray result = new JSONArray();
        for (NovaTool tool : tools.values()) {

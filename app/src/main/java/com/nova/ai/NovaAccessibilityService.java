...
public final class NovaAccessibilityService extends AccessibilityService {
    ...
    public String screenText() {
        ...
        if (root == null) return "";
        StringBuilder out = new StringBuilder(); collectText(root, out, 0); return out.toString().trim();
    }

    public String uiSnapshot() {
        AccessibilityNodeInfo root = getRootInActiveWindow();
        if (root == null) return "No accessibility window.";
        StringBuilder out = new StringBuilder(); collectUi(root, out, 0);
    ...
    private void collectText(AccessibilityNodeInfo n, StringBuilder out, int depth) {
        if (n == null || depth > 20 || out.length() > 7000) return;
    ...
    private void collectUi(AccessibilityNodeInfo n, StringBuilder out, int depth) {
        ...
        CharSequence t=n.getText(),d=n.getContentDescription();
        if((t!=null&&t.length()>0)||(d!=null&&d.length()>0)||n.isClickable()||n.isEditable()){
            Rect r=new Rect();n.getBoundsInScreen(r);
            out.append("• ").append(t!=null&&t.length()>0?t:d==null?"[unlabeled]":d)
                    .append(" | clickable=").append(n.isClickable()).append(" | editable=").append(n.isEditable())
                    .append(" | enabled=").append(n.isEnabled()).append(" | bounds=").append(r.toShortString()).append('\n');
        }
        for(int i=0;i<n.getChildCount();i++)collectUi(n.getChild(i),out,depth+1);
    }

    public boolean clickText(String text) {
        AccessibilityNodeInfo root=getRootInActiveWindow(); if(root==null||text==null||text.trim().isEmpty())return false;
        AccessibilityNodeInfo target=find(root,text.trim().toLowerCase(Locale.ROOT),0); return target!=null&&click(target);
    }

    public boolean typeText(String text) {
        if(text==null)return false;
        AccessibilityNodeInfo root=getRootInActiveWindow(); if(root==null)return false;
        AccessibilityNodeInfo target=findEditable(root,0); if(target==null)return false;
    ...
    private AccessibilityNodeInfo find(AccessibilityNodeInfo n,String q,int depth){
        ...
        for(int i=0;i<n.getChildCount();i++){AccessibilityNodeInfo x=find(n.getChild(i),q,depth+1);if(x!=null)return x;}return null;
    }

    private AccessibilityNodeInfo findEditable(AccessibilityNodeInfo n,int depth){
        if(n==null||depth>20)return null;
        if(n.isVisibleToUser()&&n.isEnabled()&&n.isEditable())return n;
        for(int i=0;i<n.getChildCount();i++){AccessibilityNodeInfo x=findEditable(n.getChild(i),depth+1);if(x!=null)return x;}return null;
    }
    ...
    public boolean swipe(float dx,float dy){
        int w=getResources().getDisplayMetrics().widthPixels,h=getResources().getDisplayMetrics().heightPixels;float cx=w/2f,cy=h/2f;
